...
# File paths
bedgraph_file_path = "../../../mapping_project/zip3_wt/peakcalling/dataset1_bdgcmp.bdg"
gff_file_path = "../../../mapping_project/zip3_wt/peakcalling/SK1.all_feature.gff"
...
def main(bedgraph_file, extend, num_realisation, feature_file, output_tag):
    ...
    with open(f"{output_tag}_enddata_ext-{str(extend)}_rdna.txt", "w") as f:
    ...
    # Writing results to output file
    with open(f"{output_tag}_endrandratio_num-real-{str(num_realisation)}_ext-{str(extend)})_rdna.txt", "w") as f:
        f.write("\n".join(map(str, rand_ratio)))

main(bedgraph_file_path, 50000, 5000, gff_file_path, "zip3_wt")
